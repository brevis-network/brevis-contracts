...
async function deployVerifier(admin: ContractRunner) {
  const factory = new SMTUpdateCircuitProofOnOpVerifier__factory();
...
async function deployContracts(admin: ContractRunner) {
  ...
  const factory = new SMT__factory();
  return factory
    .connect(admin)
    .deploy([1], [await anchorBlocks.getAddress()], [await verifier.getAddress()], [depth2EmptySmtRoot]);
}
