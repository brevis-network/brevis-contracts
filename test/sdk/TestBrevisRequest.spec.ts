...
async function deployBrevisRequestContract(admin: ContractRunner) {
  ...
  const factory = new BrevisRequest__factory();
  const brevisRequest = await factory.connect(admin).deploy(smtAddress, brevisProofAddress);
...
describe('Brevis Reqeust Test', async () => {
  async function fixture() {
    ...
    const { brevisProof, brevisRequest } = await deployBrevisRequestContract(admin);
    return { brevisProof, brevisRequest };
  }
