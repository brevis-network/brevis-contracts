...
async function deployContract(admin: Wallet) {
    const _factory = await ethers.getContractFactory<AggregationVerifier__factory>('AggregationVerifier');
...
describe('BN254 final_emulate proof verifier', async () => {
    ...
    it("should pass on verify raw with real data", async () => {
        ...
        const result = await contract.verifyRaw(
            hexToBytes("0x"+ hexValues)
        )
