...
async function deployTestBrevisAppContract(admin: Wallet) {
    ...
    const brevisProofFactory = await ethers.getContractFactory<BrevisProof__factory>('BrevisProof');
    const brevisProof = await brevisProofFactory.connect(admin).deploy(smt.address);
    const verifierF = await ethers.getContractFactory<BatchZkProofVerifier__factory>('BatchZkProofVerifier');
    const verifier = await verifierF.connect(admin).deploy();
    const verifierFApp = await ethers.getContractFactory<AggregationVerifier__factory>('AggregationVerifier');
    ...
    const factory = await ethers.getContractFactory<TestBrevisApp__factory>('TestBrevisApp');
